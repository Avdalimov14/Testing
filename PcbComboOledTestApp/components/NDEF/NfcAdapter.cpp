...
bool NfcAdapter::initRfField ()
{
	return shield -> inListPassiveTarget(true);
}

bool NfcAdapter::inDataExchange(uint8_t *send, uint8_t sendLength, uint8_t *response, uint8_t *responseLength)
{
	return shield->inDataExchange(send, sendLength, response, responseLength);
}
