...
#endif
        hwspi._spi->begin();
//        hwspi._spi->setDataMode(SPI_MODE0);  // PN532 only supports mode0
...
inline void Adafruit_SPITFT::SPI_BEGIN_TRANSACTION(void) {
    if(connection == TFT_HARD_SPI) {
        ...
        Serial.print("SPITFT::SPI_BEGIN_TRANSACTION...");
...
#elif defined(ESP8266) || defined(ESP32)
        hwspi._spi->write(b); // check which one is in use of NFC libs
#else
        hwspi._spi->write(b);
#endif
    } else if(connection == TFT_SOFT_SPI) {
